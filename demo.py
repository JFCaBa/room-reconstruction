...
def main():
    ...
    parser.add_argument("--save_npz", type=str, default=None,
                        help="Save per-frame extrinsics/intrinsics + world_points(+conf) to this "
                             ".npz for offline metric fusion, then exit (skips the 3D viewer)")
    ...
    # ── Optional: save raw geometry for offline metric fusion, then exit ──────
    if args.save_npz:
        def _to_np(x):
            if isinstance(x, torch.Tensor):
                x = x.detach().cpu().numpy()
            x = np.asarray(x)
            if x.ndim >= 1 and x.shape[0] == 1:   # drop a singleton batch dim
                x = x[0]
            return x
        out = {}
        for k in ("extrinsic", "intrinsic", "world_points", "world_points_conf",
                  "depth", "depth_conf"):
            v = predictions.get(k)
            if v is not None:
                out[k] = _to_np(v)
        out["paths"] = np.array([os.path.basename(p) for p in paths])
        npz_path = os.path.abspath(args.save_npz)
        os.makedirs(os.path.dirname(npz_path) or ".", exist_ok=True)
        np.savez_compressed(npz_path, **out)
        shapes = {k: getattr(v, "shape", None) for k, v in out.items()}
        print(f"Saved geometry -> {npz_path}\n  arrays: {shapes}")
        return
